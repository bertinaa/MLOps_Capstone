...
def campaign_simulation():
    ...
    # Plot heatmap and convert to base64
    fig, ax = plt.subplots(figsize=(8,6))
    cax = ax.imshow(roi_pivot, cmap="coolwarm", aspect="auto", origin="lower")
    ax.set_title("ROI Simulation: Discount Campaign on Top 10% Customers")
    ax.set_xlabel("Uplift in Spend")
    ax.set_ylabel("Response Rate")
    ax.set_xticks(ticks=np.arange(len(uplift_levels)))
    ax.set_xticklabels([f"{u:.0%}" for u in uplift_levels])
    ax.set_yticks(ticks=np.arange(len(response_rates)))
    ax.set_yticklabels([f"{r:.0%}" for r in response_rates])
    fig.colorbar(cax, label="ROI")
    plt.tight_layout()
    ...
    <html>
        ...
        <body>
            <h2>💰 Campaign Simulation: ROI on Top 10% Customers</h2>
